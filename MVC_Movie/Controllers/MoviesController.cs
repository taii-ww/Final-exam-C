...
namespace MVC_Movie.Controllers
{
    ...
    public class MoviesController : Controller
    {
        ...
        public ActionResult Save(Movie movie, HttpPostedFileBase imageFile)
        {
            ...
            if (movie.Id == 0)
            {
                ...
                if (imageFile != null && imageFile.ContentLength > 0)
                {
                    ...
                    var savePath = Path.Combine(Server.MapPath("~/Content/images/movies"), fileName);
                    Directory.CreateDirectory(Server.MapPath("~/Content/images/movies"));
                    imageFile.SaveAs(savePath);
                    movie.ImagePath = "/Content/images/movies/" + fileName;
                }
            ...
            else
            {
                ...
                if (imageFile != null && imageFile.ContentLength > 0)
                {
                    ...
                    Directory.CreateDirectory(Server.MapPath("~/Content/images/movies"));
                    imageFile.SaveAs(savePath);
                    movieInDb.ImagePath = "/Content/images/movies/" + fileName;
                }
            }
        ...
        // Xóa [Authorize(Roles = RoleName.CanManageMovies)] trước Index
        public ViewResult Index(int? pageIndex, string sortBy)
        {
            if (User.IsInRole(RoleName.CanManageMovies))
                return View("List");        // Admin thấy List (có Edit/Add)

            return View("ReadOnlyList");    // User thường chỉ xem
        }
